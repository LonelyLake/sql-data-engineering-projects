...
SELECT
    '2026-01-01 00:00:00+00'::TIMESTAMPTZ AT TIME ZONE 'CET';

SELECT
    '2026-01-01 00:00:00+00'::TIMESTAMPTZ AT TIME ZONE 'EET';

SELECT
    job_title_short,
    job_location,
    job_posted_date AT TIME ZONE 'UTC' AT TIME ZONE 'EET' AS job_posted_date_local
FROM
    job_postings_fact
WHERE job_location LIKE 'Helsinki, Finland';

SELECT
    EXTRACT(
        HOUR FROM job_posted_date AT TIME ZONE 'UTC' AT TIME ZONE 'EET'
    ) AS job_posted_hour,
    COUNT(job_id) AS job_count
FROM
    job_postings_fact
WHERE job_location LIKE 'Helsinki, Finland'
GROUP BY 
    EXTRACT(HOUR FROM job_posted_date AT TIME ZONE 'UTC' AT TIME ZONE 'EET')
